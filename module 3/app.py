...
class App:
    # Новая цветовая схема (сине-серая)
    COLORS = {
        'bg_primary': '#2C3E50',        # Тёмно-синий фон
        'bg_secondary': '#34495E',       # Средне-синий
        'bg_card': '#3D566E',            # Карточки
        'accent': '#3498DB',              # Ярко-синий акцент
        'accent_hover': '#5DADE2',
        'accent_light': '#EBF5FB',
        ...
        'text_secondary': '#BDC3C7',       # Светло-серый
        'border': '#7F8C8D',               # Границы
        'hover': '#5D6D7E'                  # Ховер эффект
    }
    
    # Цвета статусов (в тон палитре)
    STATUS_COLORS = {
        'Новая заявка': '#3498DB',        # Синий
        'В процессе ремонта': '#F39C12',  # Оранжевый
        'Ожидание запчастей': '#E74C3C',   # Красный
        'Готова к выдаче': '#27AE60',      # Зелёный
        'Завершена': '#95A5A6'             # Серый
    }
    ...
    def __init__(self):
        ...
        self.window = tk.Tk()
        self.window.title("Автосервис - Синяя тема")
    ...
    def show_main_menu(self):
        ...
        # Размещаем кнопки в сетке 2×N
        for i, (text, cmd, color) in enumerate(all_buttons):
            row = i // 2
            col = i % 2
            btn_frame = tk.Frame(menu_frame, bg=self.COLORS['bg_primary'])
        ...
        for r in range((len(all_buttons) + 1) // 2):
    ...
    def show_qr_code(self, req):
        try:
            ...
            messagebox.showinfo("Информация", "Модуль qrcode не установлен. Функция временно отключена.")
